...
getFile<-function(fname)
{
  ...
  if (cols != 6){ #side-effects. eesh. sorry
    flog.warn(paste("csv file doesn't have 13 columns:", fname,"column count:",cols))
    return()
  }
...
if(datediff==1){
  ...
  covid19uk <-covid19uk %>% add_row(Date=cv_scrape$report_date, Confirmed=cv_scrape$pos_tested, All_tests=cv_scrape$tested, Fatalities=cv_scrape$died, .before = 1)
  # add it to csv file
  write.table(covid19uk, file = csvfilename,row.names=FALSE, na="",col.names=TRUE, sep=",")
}else if(datediff > 1){
