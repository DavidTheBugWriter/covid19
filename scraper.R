  ...
  rm(scr)
    ...
    scrape<-function()
    {
      ...
      pat_alldate<-"^As of 9am on (.*), [0-9,]+ .*"
      #pat_pos<-"^([0-9,]+) people .* of whom ([0-9,]+) tested positive."
      pat_dieddate<-"^As of 5pm on (.*), .* tested positive for coronavirus, [0-9,]+ have died."
...
#      } else if (NA %in% positive_text){
#      flog.error("scraper failed with one or more NA terms in 'positive_text', please check")
      } else if (NA %in% death_text){
      flog.error("scraper failed with one or more NA terms in 'death_text', please check")        
      }
      ...
      scrape<-data.frame(
        ...
        #pos_tested=as.integer(gsu b(",","",positive_text[3])),
